...
class DualEncoder(nn.Module):
    def __init__(self,cont_dim, n_label=10):
        super().__init__()
        ...
        self.head = nn.ModuleList([ nn.Sequential(
            ...
            nn.Flatten(),
            nn.Linear( 4096, cont_dim, bias=False)
            ),
            nn.Sequential(
            nn.Conv2d(512, 1, kernel_size=1, padding=0),
            nn.Flatten(),
            nn.Linear(1024, cont_dim, bias=False)
            ),
            nn.Sequential(
            nn.Conv2d(512, 1, kernel_size=1, padding=0),
            nn.Flatten(),
            ...
            nn.Conv2d(512, 1, kernel_size=1, padding=0),
            nn.Flatten(),
            nn.Linear(16, cont_dim, bias=False)
            ),                         
            nn.Sequential(
            nn.Conv2d(512, 1, kernel_size=1, padding=0),
            nn.Flatten(),
            nn.Linear(1, cont_dim, bias=False)
            )])
    ...
    def forward(self, input, step=0, alpha=-1,mode="dual"):
        Head_list={}
        counter=0
...
class Encoder(nn.Module):
    def __init__(self , zdim , n_label=10):
        super().__init__()
        ...
        self.last_layer = nn.Linear(512, (zdim-1)*2)
