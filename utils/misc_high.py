...
def f_recon(img, netE, netD, zdim,label,step,alpha, mode="train", clipping=False):
    bs, imsize = img.shape[0], img.shape[2]
    mu_logvar = netE(img,step, alpha).view(bs,-1)
    mu = mu_logvar[:,0:zdim]
    ...
    if mode=="train":
        ...
        recon = netD(z,label, step, alpha).view(bs,3,imsize,imsize)
    else:
        if clipping: mu = torch.clamp(mu, min=-1.0, max=1.0)
        recon = netD(mu,label, step, alpha).view(bs,3,imsize,imsize)
...
def log_p_x_and_z(z, img, netD, zdim, mode="train", clipping=False, n_samples=1):
    ...
    if mode=="train":
        if clipping: z = torch.clamp(z, min=-1.0, max=1.0)
        recon = netD(mu,label, step, alpha)
